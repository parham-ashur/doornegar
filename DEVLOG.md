...
- **Significant-update detection** (extension of the rotation rule): a story can stay in hero/blindspot across days ONLY if its narrative shifted meaningfully. New `app/services/story_freshness.py` detector with three thresholds — dispute_score delta ≥ 0.2, any inside/outside border pct delta ≥ 15pp, or ≥3 new articles + rewritten `bias_explanation_fa` hash. New `Story.analysis_snapshot_24h` JSONB column (self-creating via `ALTER TABLE IF NOT EXISTS` idiom, no Alembic needed) refreshed by a new `step_snapshot_analyses` maintenance step at the end of every nightly run. `StoryBrief.update_signal` exposes `{has_update, kind, reason_fa}`. Hero picker now prefers fresh+has_update+balanced; blindspot picker same. Orange "بروزرسانی" badge renders next to hero title and on blindspot cards with the delta reason in Farsi. First-day behavior is conservative (no snapshots yet → no badges) until tonight's 4am UTC maintenance run populates snapshots.

### Commits (in order)
0e94bb3 · 96a5a99 · d3ba87a · 030b76c · 0ba518b · 656aa9e · 879b7ea · 40e73c5 · 5ffe35c · d4e1575 · 15d7b61 · 5e3d5b3 · dce2f86 · 743b903 (docs) · b74ec4c (rotation) · 4fb3e94 (docs) · 5388a30 (significant-update)
