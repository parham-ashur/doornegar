...
13. **Translation parity (top stories only)** — if `title_en` looks frozen or out-of-date relative to a recently-rewritten `title_fa`, fix both together: `rename_story` with `new_title_fa` + `new_title_en`. (2026-07-15: `new_title_en` on `rename_story` was silently a no-op until that day — the field was accepted in fix_data but apply_fix() never read it, so any past attempt at this workaround quietly failed. Fixed in journalist_audit.py; verified working.)
