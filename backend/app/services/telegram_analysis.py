...
{{
  ...
  "predictions": [
    {{"text": "پیش‌بینی = گزاره‌ای دربارهٔ اتفاقِ آینده. فعلِ آینده‌دار دارد (خواهد، می‌شود، رخ می‌دهد). ۴ تا ۶ پیش‌بینی متمایز بده — نه یکی، نه دو تا. ممنوع: ذکرِ نامِ کانال در متن («کانال X پیش‌بینی کرد»)، نقل‌قول مستقیم («گفت که …»)، یا پیش‌بینیِ فرا-رسانه‌ای («کانال‌های اپوزیسیون Y خواهند گفت»). فقط جوهرِ اتفاق را بنویس: «آتش‌بس تا پایان ماه می‌شکند»، «تلفات از ۵۰۰ نفر می‌گذرد»، «مذاکرات شکست می‌خورد».", "supporters": ["نامِ دقیقِ کانال‌هایی که این پیش‌بینی را مطرح کرده‌اند — همان عنوانی که داخل کروشهٔ [...] در فهرستِ «پست‌های خام» بالا آمده. ۱ تا ۴ نام. این فهرست در صفحهٔ اصلی سایت زیرِ هر پیش‌بینی نمایش داده می‌شود — لیستِ خالی به‌معنای «بدون منبع» است و پیش‌بینی نمایش داده نمی‌شود، پس حتماً پر کن. اگر هیچ کانالی قابل‌انتساب نیست، آن پیش‌بینی را اصلاً نیاور."]}},
    {{"text": "پیش‌بینی دوم دربارهٔ رویدادی متفاوت از پیش‌بینی اول — تکرار ممنوع", "supporters": ["نام کانال X از فهرست بالا", "نام کانال Y از فهرست بالا"]}},
    {{"text": "پیش‌بینی سوم", "supporters": ["نام کانال Z از فهرست بالا"]}},
    {{"text": "پیش‌بینی چهارم", "supporters": ["نام کانال W از فهرست بالا"]}}
  ],
  "worldviews": {{
  ...
  "key_claims": [
    "گزاره‌ای دربارهٔ گذشته یا حالِ واقعی که یک طرف مدعی دانستنِ آن است و بقیه نمی‌دانند یا انکار می‌کنند. فعلِ آینده ممنوع — اگر فعل آینده دارد، به predictions منتقل کن. ممنوع‌های نگارشی: (۱) پیشوندِ «ادعا:» یا «موضوع: X |» در ابتدا، (۲) قرار دادنِ متنِ ادعا داخل «...» یا \"...\"، (۳) ذکرِ نامِ کانال یا «کانال X» در هر جایِ متن، (۴) دنبالهٔ «— کانال Y» یا «— ارزیابی: …» در انتها، (۵) نقل‌قول مستقیم با افعالِ «گفت/نوشت/ادعا کرد». فقط جوهرِ خام را بنویس، بدون پیشوند، بدون گیومه، بدون انتساب: ۵۰۰ موشک در حملهٔ دیشب شلیک شده. فرماندهٔ سپاه کشته شده. مذاکرات مخفیانه در عمان در جریان است.",
    "ادعای دوم. اگر چند طرف دربارهٔ یک موضوع (مثلاً شمارِ تلفات) ارقام متفاوت می‌دهند، هر دو ادعا را بیاور تا خواننده مقایسه کند — بدون نام کانال."
  ],
...
async def reassign_posts_by_embedding(
    *,
    ...
    if _ms is not None:
        await _ms.update_step_progress(total_posts, total_posts, label="flushing updates")
