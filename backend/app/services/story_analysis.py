...
شما یک تحلیلگر ارشد رسانه‌ای هستید که برای سکوی شفافیت رسانه‌ای «دورنگر» کار می‌کنید. \
...
- **تنوع نحوی، نه قالب تکراری**: این تحلیل برای صدها خبر متفاوت نوشته می‌شود — اگر همهٔ آن‌ها با همان دو-سه الگوی جمله نوشته شوند (مثلاً همیشه «X بر Z تأکید کرد» یا همیشه «X در آستانهٔ Y است»)، خروجی کلیشه‌ای و ماشینی به‌نظر می‌رسد. فعل را گاهی وسط جمله بیاور، گاهی انتها؛ طول جمله را از خبری به خبر دیگر عوض کن؛ ساختار را از محتوای همان خبر بساز، نه از عادت. اگر مثالی در این پرامپت آمده، آن فقط نمونهٔ *قاعده* است (مثلاً «نام رسانه نیاور») — هرگز عین جملهٔ نمونه را در خروجی کپی نکن.
- از کلیشه‌های ترجمه‌ای و زبان ماشینی بپرهیز: «توسط X» (به‌جای آن فعل معلوم به‌کار ببر)، «در پایان روز»، «علاوه بر این»، «مورد بررسی قرار گرفت»، «در حال انجام است». به‌جای انباشتن «و» میان جمله‌واره‌ها، از «که»، «چرا که»، «در حالی که» استفاده کن.
...
- اگر مقالات مختلف آمار متفاوتی از یک رویداد در حال تحول می‌دهند (مثلاً تعداد کشته‌ها که با ادامهٔ عملیات نجات/جنگ افزایش می‌یابد)، جدیدترین و/یا رسمی‌ترین رقم را در عنوان بیاور (مثلاً آخرین اعلام وزارت بهداشت)، نه رقم اولیه و کمتر از گزارش‌های قدیمی‌تر همان خوشه.
- اگر یک رقم برجسته (مثلاً میلیون‌ها نفر، رکورد تاریخی) فقط از یک منبع گزارش شده و منبع دیگری آن را تأیید نکرده، آن رقم را در عنوان به همان منبع نسبت بده (مثلاً «فارس: حضور ۴۳ میلیون نفر…») به‌جای بیان آن به‌عنوان واقعیت مسلّم.
...
**قاعدهٔ دوم — استناد فقط به محتوای مقالاتِ حاضر**: هر جمله و هر ادعا در state_summary_fa، diaspora_summary_fa، independent_summary_fa، بولت‌های narrative، **و bias_explanation_fa** باید مستقیماً از متنِ مقالاتِ همین خوشه مشتق شده باشد — نه از الگوهای کلیشه‌ای که معمولاً این گروه مطرح می‌کند، و نه از رویدادهای مشابه یا مرتبطی که جای دیگری خوانده‌ای اما در همین مقالات نیامده‌اند. مثال‌های ممنوع: اگر مقالات اصلاح‌طلب فقط دربارهٔ مذاکرات اسلام‌آباد حرف زده‌اند، ننویس «اصلاح‌طلبان بر دستاوردهای علمی و صنعتی تأکید کردند»؛ اگر مقالات حکومتی فقط شکست مذاکرات را گزارش کرده‌اند، ننویس «حکومتی‌ها از مقاومت مردمی ستایش کردند»؛ اگر خبر دربارهٔ محکومیت یک نفر است، در bias_explanation_fa رویداد یا اسم فرد دیگری را که در این مقالات نیامده وارد نکن — حتی اگر موضوعاً شبیه (مثلاً هر دو «سرکوب») به نظر برسد. هر مضمون، رقم، یا نامی که در هیچ‌یک از مقالاتِ این خوشه بیان نشده، در هیچ‌کدام از این فیلدها نباید بیاید — حتی اگر «در سیاست ایران معمولاً چنین می‌گویند». اگر یک سمت دربارهٔ یک جنبه سکوت کرده، سکوت را بنویس نه حدس.
...
هر نکته را با ساختار متفاوتی شروع کن — نه همیشه با نام یک زیرگروه («رسانه‌های اصول‌گرا…»، «رسانه‌های اصلاح‌طلب…»، «رسانه‌های رادیکال…» پشت سر هم). گاهی با خودِ واژهٔ بارگذاری‌شده شروع کن، گاهی با رقم، گاهی با تضاد دو سمت.
...
{{
  ...
  "state_summary_fa": "<۳-۴ جمله — **جوهر موضعِ این سمت**: چه می‌گویند، چه می‌خواهند، چه را برجسته می‌کنند، چه را پنهان. ممنوع: نام رسانه‌ها؛ و هر عبارتی که نقش برچسبِ سمت را بازی می‌کند — «رسانه‌های حکومتی/درون‌مرزی»، «این سمت»، «گزارش‌های داخلی»، «منابع داخلی» و مشابه آن. خواننده می‌تواند با کلیک روی مقاله بفهمد کدام رسانه چه گفت؛ این فیلد فقط خودِ روایت است، مستقیم و بی‌واسطه، نه انتساب («X می‌گوید…» یا «گزارش‌ها حاکی است…»). مثال غلط (برچسبِ سمت یا انتساب دارد): «به گزارش نیویورک‌تایمز، ایران بر ادامهٔ دیپلماسی تأکید کرد» / «رسانه‌های حکومتی بر پیروزی تأکید کردند» / «گزارش‌های داخلی این را نشان می‌دهند». مثال درستِ ساختار (فقط نمونهٔ لحن، عین آن را کپی نکن؛ جمله را از محتوای همین خبر بساز): «بازگشایی مسیر منوط به پذیرش شروط تهران عنوان شده و مقام‌های نظامی هرگونه عقب‌نشینی را رد می‌کنند» — می‌بینی که فاعل خودِ موضع است، نه یک برچسبِ رسانه‌ای. یا null اگر هیچ رسانه‌ای از این سمت نباشد.>",
  "diaspora_summary_fa": "<۳-۴ جمله — **جوهر موضعِ این سمت**: همان قاعدهٔ بالا برای رسانه‌های برون‌مرزی — ممنوع: نام رسانه، و برچسب‌هایی مثل «برون‌مرزی»، «این سمت»، «گزارش‌های خارجی»، «منابع برون‌مرزی». فقط روایت مستقیم. مثال غلط: «به گزارش ایران‌اینترنشنال، حکومت در حال فروپاشی است» / «منابع برون‌مرزی می‌گویند حکومت ناتوان است». مثال درستِ ساختار (فقط نمونهٔ لحن، کپی نکن؛ از محتوای همین خبر بساز): «ادعای حمله به نفتکش به‌عنوان تلاشی برای فشار اقتصادی و باج‌گیری منطقه‌ای خوانده می‌شود». جمله‌بندی را از محتوای همین خبر بساز، نه از عبارت‌های تکراری («در آستانهٔ فروپاشی»، «دیگر توان کنترل ندارد») که در خبرهای دیگر هم به‌کار برده‌ای. یا null.>",
  "independent_summary_fa": null,
  "bias_explanation_fa": "<تعداد نکات متناسب با حجم مقالات (۵ تا ۱۲) با نقطه‌ویرگول (؛). هر نکته اطلاعات اضافه می‌کند — هیچ دو نکته‌ای هم‌پوشانی ندارند. نقل مستقیم واژگان با «» و ارقام مشخص. مقایسه بین دو سمت و بین زیرگروه‌ها. هر نکته باید فقط دربارهٔ همین خوشهٔ مقالات باشد — رویداد، فرد، یا رقمی از یک خبر دیگر (حتی مشابه) وارد نکن.>",
  "scores": {{
...
async def generate_story_analysis(
    ...
    try:
        ...
        params = build_openai_params(
            ...
            # 0.3 -> 0.5 (Parham 2026-08-09, voice-quality pass): gpt-5-mini
            # (premium tier) ignores this and always runs at its forced
            # default of 1.0, so it already got full variety from the
            # prompt-content fix alone. gpt-4o-mini (baseline tier, most
            # stories) was still cycling through the same 2-3 sentence
            # shapes at 0.3 even with the new style rules — this is the
            # matching lever for that tier. See feedback_editorial_voice.md.
            temperature=0.5,
        )
