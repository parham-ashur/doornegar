...
- summary_fa: ۱ تا ۲ جمله کوتاه. حدود ۲۰ تا ۳۰ کلمه. فقط حقایق اصلی (برای نمایش در کارت صفحه اصلی).
- state_summary_fa, diaspora_summary_fa, independent_summary_fa: ۲ تا ۳ جمله. حدود ۲۵ تا ۴۰ کلمه.
- bias_explanation_fa: ۳ تا ۵ نکته مجزا، هر کدام یک جمله. با نقطه‌ویرگول (؛) جدا کن. حدود ۵۰ تا ۸۰ کلمه.
...
# عنوان به‌روز (مهم — این عنوان در صفحه اصلی نمایش داده می‌شود)

عنوان باید مانند تیتر روزنامه باشد: کوتاه، خبری، و حاوی جزئیات کلیدی.
- title_fa: عنوان فارسی (حداکثر ۱۵ کلمه). جزئیات مهم را بگنجان: ارقام، نام‌ها، شرایط.
  مثال‌های خوب: «آتش‌بس دو هفته‌ای ایران و آمریکا؛ طرح ۱۰ ماده‌ای پاکستان» یا «حمله به دانشگاه شریف؛ ۲۸ کشته و واکنش سپاه»
  مثال بد: «توافق آتش‌بس بین ایران و آمریکا و واکنش‌های جهانی» (خیلی کلی، بدون جزئیات)
- title_en: ترجمه انگلیسی عنوان.
- خلاصه جداگانه ننویس — عنوان باید خودش خلاصه رویداد باشد.
