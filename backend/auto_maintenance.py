...
وظیفه: تمیزکاری ادعاها و پیش‌بینی‌های تلگرامی برای نمایش در صفحه اصلی. متن اصلی توسط مدل دیگری تولید شده و شامل برچسب‌ها و افعالِ مقدمه‌ای اضافی است که باید حذف شوند. خواننده صفحه اصلی فقط چند ثانیه وقت دارد — جوهرِ ادعا و پیش‌بینی را مستقیم به او بده، نه مقدمه‌چینی را.
...
- **حذف افعالِ مقدمه‌چینی و انتسابِ مبهم**. اینها حذفی‌اند:
    ...
    * «کانال‌های مخالف گفتند …»
    * «رسانه‌های تلگرامی نوشتند …»
    * «به گفتهٔ کانال X، …»
  بدل کن به: فقط ادعا + برچسب اعتبار/جهت‌گیری در صدر.
- **حذف پیشوندهای برچسبی** از متن اصلی: «موضوع: … |»، «تعداد تلفات: N |»، «ادعا: …»، «پوشش: …» — این‌ها دسته‌بندی داخلی بودند، نه محتوا.
- **استفاده از برچسب در صدر جمله وقتی ممکن است**. ساختار مرجّح: «[برچسب]: [ادعا]». برچسب‌های مجاز (یکی، در صدر، پررنگ):
    * «تأیید شده:» — وقتی چند منبع مستقل رقم یا واقعه را تأیید کرده‌اند
    * «مشکوک:» — وقتی منبع مستقل تأیید نکرده یا ارقام متناقض‌اند
    * «تبلیغاتی:» — وقتی لحن و واژگان آشکارا تبلیغاتی‌اند
    * «تک‌منبع:» — وقتی فقط یک کانال ادعا کرده و تکرار یا تأیید نشده
    * «نیازمند تأیید:» — وقتی در مرحلهٔ ابهام است و هنوز منابع مستقل وارد نشده‌اند
- اگر برچسب از روی متن اصلی قابل تشخیص نیست، بدون برچسب بنویس — جعل برچسب ممنوع.
- نام یک کانال فقط وقتی بیاید که ادعا به‌طور انحصاری از آن آمده و هیچ تأییدی نشده، آن هم در انتهای جمله: «… (فقط کانال [نام])». در غیر این صورت، نیازی به نام بردن نیست.

قوانین کلی:
- بدون افزودن معنای جدید — فقط تمیز کن و بازسازی کن.
- اگر متن اصلی خالی یا نامفهوم است، همان متن اصلی را برگردان.
- ساختار جمله فارسی باشد (فعل در انتها، اتصال با «که»/«چرا که»)؛ بدون ترجمهٔ تحت‌اللفظی انگلیسی.
- خروجی فقط JSON، بدون توضیح.

═══ نمونه‌های قبل / بعد ═══

پیش‌بینی قبل: «احتمالاً در هفته‌های آینده مذاکرات مجدد و تلاش برای نزدیک کردن مواضع طرفین رخ خواهد داد.»
پیش‌بینی بعد: «در هفته‌های آینده مذاکرات مجدد و تلاش برای نزدیک کردن مواضع طرفین رخ خواهد داد.»

ادعا قبل: «موضوع: نتیجه مذاکرات اسلام‌آباد | کانال آخرین خبر ادعا کرد مذاکرات پس از ۲۱ ساعت به بن‌بست رسید — معتبر، زیرا منابع رسمی و چندین کانال هم‌نظر هستند.»
ادعا بعد: «تأیید شده: مذاکرات اسلام‌آباد پس از ۲۱ ساعت به بن‌بست رسید.»

ادعا قبل: «کانال‌های حکومتی اعلام کردند که نیروهای آمریکا در عراق شکست سنگین خورده‌اند.»
ادعا بعد: «تبلیغاتی: نیروهای آمریکا در عراق شکست سنگین خورده‌اند.»

ادعا قبل: «کانال وحید آنلاین گفت ونس خطوط قرمز آمریکا را مشخص کرده و حالا نوبت تهران است — قابل‌اعتماد.»
ادعا بعد: «تأیید شده: ونس خطوط قرمز آمریکا را مشخص کرد؛ نوبت تهران است.»
