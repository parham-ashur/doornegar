...
═══ قواعد مشترک ═══
- **انتساب کانال در پرانتز در انتها**. هر آیتم باید با نام کانال در پرانتز پایان یابد، مثلاً «… (کانال مصاف)» یا «… (وحید آنلاین)». اگر در ورودی چند کانال آمده، با علامت ÷ جدا کن: «… (کانال X ÷ کانال Y)». اگر کانال در ورودی نیست، پرانتز نگذار.
- **حذف فعل انتسابی از داخل متن**. «گفت که …»، «نوشت …»، «اعلام کرد …» — همه از داخل جمله حذف. فقط جوهر ادعا یا پیش‌بینی در جمله، نام کانال فقط در پرانتزِ انتها.
- بدون افزودن معنای جدید — فقط بازآرایی کن.
- خروجی فقط JSON.
...
- **انتسابِ کانال فقط در انتها در پرانتز**: ورودی‌های مثل «— کانال X»، «— کانال X نقل قول از Y»، «— کانال X ارجاع به Y»، «— کانال‌های …» را به فرم «(کانال X)» در انتها تبدیل کن. اگر چند کانال هست، با ÷ جدا کن.
...
ادعا بعد: «تأیید شده: آمریکا و ایران خواهان بازگشت به جنگ نیستند، اما اعتمادی وجود ندارد. (ML Strategy)»
...
ادعا بعد: «تبلیغاتی: ترامپ با اعمال محاصره و نقض آتش‌بس می‌خواهد میز مذاکره را به میز تسلیم تبدیل کند. (ML Strategy)»
...
ادعا بعد: «تأیید شده: ما مذاکره زیر سایه تهدید را نمی‌پذیریم. (farahmand_alipour)»
...
تعداد آیتم‌های خروجی باید دقیقاً با تعداد ورودی برابر باشد، به همان ترتیب."""
    ...
    for story in stories:
        ...
        # PROMPT_VERSION bumps invalidate every stored digest so all
        # stories re-polish on the next tick — used when the prompt
        # itself changes (e.g., 2026-04-25 channel-attribution restore).
        PROMPT_VERSION = "v2-channel-attribution"
        def _digest(items):
            texts = [i if isinstance(i, str) else (i.get("text") or "") for i in items]
            return "|".join(t[:40] for t in texts)

        raw_digest = f"{PROMPT_VERSION}::" + _digest(raw_preds) + "::" + _digest(raw_claims)
