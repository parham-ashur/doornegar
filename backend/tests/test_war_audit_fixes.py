...
# ═════════════════════════════════════════════════════════════════════
# Grab-bag title-judge (2026-06-20) — coherence_judge.py
# ═════════════════════════════════════════════════════════════════════

class TestCoherenceJudge:
    """The coherence gate archives grab-bag clusters using an LLM title-judge.
    Embedding geometry was tried first and reverted (grab-bags scored HIGHER
    than coherent stories). These tests pin the pure logic: parsing, the
    grab-bag threshold, and the exemptions. The LLM call itself is not tested
    (it's patched in production); the threshold was validated against a labeled
    live set on 2026-06-20 (9/9 correct)."""

    def test_parse_basic(self):
        from app.services.coherence_judge import parse_judge_response
        raw = '{"dominant_event": "X", "on_topic_count": 3, "total": 15, "off_topic_indices": [4,5]}'
        p = parse_judge_response(raw, total_sent=15)
        assert p["on_topic_count"] == 3
        assert p["total"] == 15
        assert p["dominant_event"] == "X"

    def test_parse_strips_markdown_fence(self):
        from app.services.coherence_judge import parse_judge_response
        raw = '```json\n{"dominant_event": "Y", "on_topic_count": 8, "total": 10}\n```'
        p = parse_judge_response(raw, total_sent=10)
        assert p is not None and p["on_topic_count"] == 8

    def test_parse_uses_total_sent_not_model_echo(self):
        """We trust the count of titles we actually sent, not the model's
        echoed total (which can be wrong / hallucinated)."""
        from app.services.coherence_judge import parse_judge_response
        raw = '{"dominant_event": "Z", "on_topic_count": 2, "total": 99}'
        p = parse_judge_response(raw, total_sent=9)
        assert p["total"] == 9

    def test_parse_clamps_on_topic_to_total(self):
        from app.services.coherence_judge import parse_judge_response
        raw = '{"dominant_event": "Z", "on_topic_count": 50, "total": 9}'
        p = parse_judge_response(raw, total_sent=9)
        assert p["on_topic_count"] == 9  # clamped

    def test_parse_unparseable_returns_none(self):
        """A parse failure must return None so the caller never archives
        on uncertainty."""
        from app.services.coherence_judge import parse_judge_response
        assert parse_judge_response("not json at all", total_sent=9) is None
        assert parse_judge_response("", total_sent=9) is None
        # truncated mid-string (the bug that 300-token cap caused)
        truncated = '{"dominant_event": "X", "on_topic_count": 2, "total": 9, "off'
        assert parse_judge_response(truncated, total_sent=9) is None

    def test_is_grab_bag_below_threshold(self):
        from app.services.coherence_judge import is_grab_bag
        # 2/9 = 22% < 45% → grab-bag
        assert is_grab_bag({"on_topic_count": 2, "total": 9}) is True
        # 2/5 = 40% < 45% → grab-bag (the 'internet' case)
        assert is_grab_bag({"on_topic_count": 2, "total": 5}) is True

    def test_is_grab_bag_above_threshold_kept(self):
        from app.services.coherence_judge import is_grab_bag
        # 12/24 = 50% >= 45% → coherent (the iran-2-nuclear borderline case)
        assert is_grab_bag({"on_topic_count": 12, "total": 24}) is False
        # 15/25 = 60% → coherent
        assert is_grab_bag({"on_topic_count": 15, "total": 25}) is False

    def test_is_grab_bag_none_is_false(self):
        """None (judge failed) must never be treated as a grab-bag."""
        from app.services.coherence_judge import is_grab_bag
        assert is_grab_bag(None) is False

    def test_is_grab_bag_small_story_skipped(self):
        """Below MIN_ARTICLE_COUNT we don't judge coherence."""
        from app.services.coherence_judge import is_grab_bag
        assert is_grab_bag({"on_topic_count": 0, "total": 3}) is False

    def test_threshold_separates_labeled_set(self):
        """The 2026-06-20 labeled dry-run: grab-bags 10-40% on-topic,
        coherent stories 50-67%. The 0.45 threshold must split them."""
        from app.services.coherence_judge import is_grab_bag
        grab_bags = [(1, 9), (2, 5), (3, 15), (2, 14), (1, 10)]   # 11-40%
        coherent = [(15, 25), (12, 23), (12, 24), (4, 6)]          # 50-67%
        for on, tot in grab_bags:
            assert is_grab_bag({"on_topic_count": on, "total": tot}) is True, (
                f"grab-bag {on}/{tot} must be flagged"
            )
        for on, tot in coherent:
            assert is_grab_bag({"on_topic_count": on, "total": tot}) is False, (
                f"coherent {on}/{tot} must be kept"
            )

    def test_step_in_full_pipeline_not_ingest_only(self):
        import importlib, sys
        sys.path.insert(0, str(__import__("pathlib").Path(__file__).parent.parent))
        m = importlib.import_module("auto_maintenance")
        full_keys = [k for k, _, _ in m.FULL_PIPELINE]
        ingest_keys = [k for k, _, _ in m.INGEST_ONLY_PIPELINE]
        assert "coherence_gate" in full_keys
        assert "coherence_gate" not in ingest_keys
