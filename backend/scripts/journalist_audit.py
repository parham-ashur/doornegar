  ...
  "findings": [
    {
      ...
      "fix_data": {
        ...
        "new_title_en": "ترجمه انگلیسی عنوان جدید — اختیاری، برای rename_story هم قابل استفاده است تا title_en هم‌زمان با title_fa به‌روز شود و از عقب‌ماندگی ترجمه جلوگیری شود",
    ...
    async with async_session() as db:
        if fix_type == "rename_story" and fix_data.get("new_title_fa"):
            ...
            if story:
                ...
                # 2026-07-15: title_en used to go stale on every rename_story
                # call because this was the ONLY field it touched — the sole
                # other path that can write title_en (write_preliminary_summary)
                # forces a mandatory new_summary_fa rewrite too, so a plain
                # title-only fix had no clean fix_type. Optional here: pass
                # new_title_en alongside new_title_fa when translating too.
