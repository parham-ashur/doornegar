...
import { FeedbackProvider, StoryFeedback } from "@/components/home/FeedbackOverlay";
...
function fb(locale: string, id: string) {
  return `/${locale}/stories/${id}?feedback=1`;
}

// ─── Main page (identical to homepage, wrapped in FeedbackProvider) ──
...
export default async function RatePage({
  ...
  for (const s of [...leftTextStories, ...mostViewed, ...overflow]) allStoriesForSummary.add(s.id);
  ...
  return (
    ...
    <div dir="rtl" className="mx-auto max-w-7xl px-0 md:px-6 lg:px-8">
      ...
      <div className="md:hidden">
        {/* Hero */}
        <StoryFeedback storyId={hero.id} title={hero.title_fa} imageUrl={hero.image_url}>
          <Link href={fb(locale, hero.id)} className="block border-b border-slate-200 dark:border-slate-800">
        ...
        {(conservativeBlind || oppositionBlind) && (
          <div className="px-4 py-5 border-b border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-4">
              {conservativeBlind && (
                <StoryFeedback storyId={conservativeBlind.id} title={conservativeBlind.title_fa} imageUrl={conservativeBlind.image_url}>
                  <Link href={fb(locale, conservativeBlind.id)} className="group block border-[3px] border-[#1e3a5f] shadow-[0_0_12px_rgba(30,58,95,0.4)] animate-pulse-glow-blue">
              ...
              {oppositionBlind && (
                <StoryFeedback storyId={oppositionBlind.id} title={oppositionBlind.title_fa} imageUrl={oppositionBlind.image_url}>
                  <Link href={fb(locale, oppositionBlind.id)} className="group block border-[3px] border-[#ea580c] shadow-[0_0_12px_rgba(234,88,12,0.4)] animate-pulse-glow-orange">
                    <div className="flex gap-3 p-3">
                      <div className="w-20 h-20 shrink-0 overflow-hidden bg-slate-100 dark:bg-slate-800">
          ...
          <div className="px-4 py-5 border-b border-slate-200 dark:border-slate-800">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {leftTextStories.map((s) => {
                ...
                return (
                  <StoryFeedback key={s.id} storyId={s.id} title={s.title_fa}>
                    <Link href={fb(locale, s.id)} className="group block py-4">
        ...
        {mostViewed.length > 0 && (
          <div className="px-4 py-5 border-b border-slate-200 dark:border-slate-800">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {mostViewed.map((s, i) => (
                <StoryFeedback key={s.id} storyId={s.id} title={s.title_fa}>
                  <Link href={fb(locale, s.id)} className="group flex items-start gap-3 py-3">
        ...
        {/* Telegram */}
        <div className="px-4 py-5 border-b border-slate-200 dark:border-slate-800">
          <h3 className="text-[13px] font-black text-slate-900 dark:text-white mb-3 pb-2 border-b border-slate-200 dark:border-slate-800">تحلیل روایت‌های تلگرام</h3>
      ...
      <div className="grid grid-cols-12 gap-0 border-b-2 border-slate-300 dark:border-slate-700">
        ...
        <div className="col-span-3 py-6 pl-6 border-l border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden" style={{ maxHeight: 700 }}>
          <h3 className="text-[13px] font-black text-slate-900 dark:text-white mb-3 pb-2 border-b border-slate-200 dark:border-slate-800 shrink-0">تحلیل روایت‌های تلگرام</h3>
        ...
        {hero && (
          <div className="col-span-6 py-6 px-5">
            <StoryFeedback storyId={hero.id} title={hero.title_fa} imageUrl={hero.image_url}>
              <Link href={fb(locale, hero.id)} className="group block">
                <div className="aspect-[16/9] overflow-hidden bg-slate-100 dark:bg-slate-800">
                  <SafeImage src={hero.image_url} className="h-full w-full object-cover" />
                </div>
                <h1 className="mt-4 text-[28px] font-black leading-snug text-slate-900 dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-400 line-clamp-3">{hero.title_fa}</h1>
              </Link>
            </StoryFeedback>
            <Meta story={hero} />
        ...
        <div className="col-span-3 py-4 pr-6 border-r border-slate-200 dark:border-slate-800 space-y-4 flex flex-col justify-center">
          ...
          {conservativeBlind && (
            <StoryFeedback storyId={conservativeBlind.id} title={conservativeBlind.title_fa} imageUrl={conservativeBlind.image_url}>
              <Link href={fb(locale, conservativeBlind.id)} className="group block border-[3px] border-[#1e3a5f] shadow-[0_0_12px_rgba(30,58,95,0.4)] hover:shadow-[0_0_20px_rgba(30,58,95,0.6)] transition-shadow animate-pulse-glow-blue">
          ...
          {oppositionBlind && (
            <StoryFeedback storyId={oppositionBlind.id} title={oppositionBlind.title_fa} imageUrl={oppositionBlind.image_url}>
              <Link href={fb(locale, oppositionBlind.id)} className="group block border-[3px] border-[#ea580c] shadow-[0_0_12px_rgba(234,88,12,0.4)] hover:shadow-[0_0_20px_rgba(234,88,12,0.6)] transition-shadow animate-pulse-glow-orange">
                <div className="aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
                  <SafeImage src={oppositionBlind.image_url} className="h-full w-full object-cover" />
                </div>
                <div className="p-3">
              ...
              {[mostDisputed, secondDisputed].filter(Boolean).map((story, i) => {
                ...
                return (
                  <div key={s.id} className={`py-4 ${i > 0 ? "border-t border-slate-100 dark:border-slate-800/60" : ""}`}>
                    <StoryFeedback storyId={s.id} title={s.title_fa}>
                    ...
                    {(stateSummary || diasporaSummary) && (
                      <div className="mt-2 space-y-1">
                        {stateSummary && <p className="text-[13px] leading-5 text-slate-500 dark:text-slate-400 line-clamp-1"><span className="text-[#1e3a5f] dark:text-blue-300 font-medium">• </span>{stateSummary}</p>}
                        {diasporaSummary && <p className="text-[13px] leading-5 text-slate-500 dark:text-slate-400 line-clamp-1"><span className="text-[#ea580c] dark:text-orange-400 font-medium">در مقابل </span>{diasporaSummary}</p>}
                      </div>
                    )}
                  </div>
                );
