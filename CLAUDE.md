...
- **Only `FULL_PIPELINE` runs on a cron schedule**: `0 3 * * *` UTC (1× daily, since 2026-08-13). Configured on Railway service `maintenance-cron` — **schedule lives only in the Railway dashboard, not in a repo config file**; if you need to change it, tell Parham the exact dashboard steps rather than looking for a CLI/API path. History: Phase G.3.5 (Parham 2026-05-12) reduced 3×→2× daily (`0 3,15 * * *`, 12h apart) for the ≤2 GB/day egress target. Reduced again 2×→1× on 2026-08-13 (Parham's call, to cut LLM + Neon compute cost — LLM spend roughly halves since fewer stories get processed per day; Neon compute exposure drops since the cron holds a connection for its runtime). **Tradeoff to keep in mind**: breaking news can now take up to ~24h to be ingested/clustered/appear on the homepage instead of up to ~12h — worth revisiting if a fast-moving story (e.g. an active war day) needs fresher coverage; the single fire is at 03:00 UTC (06:30 IRST, Tehran morning) so the homepage still refreshes before the day's readers arrive, just not again in the evening. Any cron-cadence change must be paired with an audit of every canary/dashboard-check with a hardcoded hour threshold (`ingest_silence`, `telegram_session`, `translation_stale_count`, `breaking_news_unclustered`, dashboard's "Last ingestion" check, `maintenance_info`'s `hours_ago < 28` fallback — all in `app/api/v1/admin.py`) — see [[feedback_canary_design]] memory, this exact class of bug was fixed 2026-08-09 and re-fixed for the 1×/day cadence 2026-08-13, with `breaking_news_unclustered` (missed in the 08-13 pass — its window encodes an assumption about how many cluster passes have happened, not just a raw staleness threshold) caught and fixed 2026-08-14.
